        ...
        <div class="statbox widget box box-shadow">
            ...
            <div class="widget-content widget-content-area mt-3">
                <div class="d-flex justify-content-between align-items-center mt-3">
                    ...
                    <a href="{{ url('admin/bar-menu-categories/create') }}" class="btn btn-primary">+ Add Category</a>
                </div>
                <table id="html5-extension" class="table dt-table-hover mt-3" style="width:100%">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $category->bar->name }}</td>
                            <td>{{ $category->name }}</td>
                            <td>
                                <button type="button" 
                                    class="btn btn-sm toggle-status-btn {{ $category->status == 1 ? 'btn-success' : 'btn-danger' }}" 
                                    data-id="{{ $category->id }}">
                                    {{ $category->status == 1 ? 'Active' : 'Inactive' }}
                                </button>
                            </td>
                            <td>
                                <a href="{{ url('admin/bar-menu-categories/'.$category->id.'/edit') }}"
                                class="btn btn-sm btn-warning">Edit</a>
                                
                                <form action="{{ url('admin/bar-menu-categories/'.$category->id) }}"
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    buttons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            const btn = this;
            fetch(`/admin/bar-menu-categories/${barId}/toggle-status`, {
                method: 'POST',
                headers: {
